...
from rasterio.io import MemoryFile # Nieuwe import voor MemoryFile
...
# --- NIEUW: Hoogte drempel voor objecten die eigen schaduw werpen ---
# Dit is een cruciale parameter. Je moet deze aanpassen op basis van wat "hoog" is in jouw DSM.
# Bijv. 5 meter boven het maaiveld om gebouwen en volwassen bomen te vangen.
object_height_threshold = 5.0 # meters boven het maaiveld
...
def calculate_shadows(dsm_data, nodata_value, transform, sun_azimuth_rad, sun_elevation_rad, object_mask):
    """
    Berekent de schaduwen op basis van een Digital Surface Model (DSM), zonpositie, transformatiegegevens,
    en een masker voor objecten die eigen schaduw werpen.
    ...
    - object_mask (np.array): Een binair masker (1 voor object, 0 voor geen object) voor pixels die
                               altijd als "schaduw" worden beschouwd (direct onder het object).
    ...
    for r in range(rows):
        for c in range(cols):
            ...
            # NIEUW: Als de pixel deel is van een "hoog object", markeer als schaduw
...
try:
    ...
    # DSM genormaliseerd zodat de laagste landhoogte 0 is.
    # Dit maakt de `object_height_threshold` relatief aan het maaiveld.
    ...
    # --- NIEUW: Creëer een masker voor objecten die eigen schaduw werpen ---
    # Pixels waar de genormaliseerde hoogte boven de drempel is, worden gemarkeerd (1 = object, 0 = geen object)
    object_mask = (dsm_normalized_for_shadow > object_height_threshold).astype(np.uint8)
    print(f"Objectmasker gemaakt met drempel van {object_height_threshold}m (genormaliseerd).")
    print(f"Aantal objectpixels: {np.sum(object_mask)}")
    ...
    for i, current_dt_local in enumerate(time_range):
        ...
        # AANPASSING: Geef object_mask mee aan calculate_shadows
        shadow_result = calculate_shadows(dsm_normalized_for_shadow, np.nan, subset_transform, sun_azimuth_rad, sun_elevation_rad, object_mask)
    ...
    heatmap_filename1 = f'schaduw_heatmap_fietspaden_looppaden_pixel_{analysis_date.strftime("%Y%m%d")}_met_objectschaduw.png'
    ...
    elements_with_avg_shadow['avg_shadow_percent'] = np.nan
    ...
    heatmap_filename2 = f'schaduw_heatmap_fietspaden_looppaden_gemiddeld_{analysis_date.strftime("%Y%m%d")}_met_objectschaduw.png'
